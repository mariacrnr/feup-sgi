...
void main() {
	...
	offset = aVertexNormal * scaleFactor * (sin(timeFactor) + 1.0) / 2.0;
    ...
    vec3 vecCoords = -vec3(vertex.xyz);
    vec3 normalVecCoords = normalize(vecCoords);

    vFinalColor = lighting(vertex, normalVecCoords, N);
